    ...
    <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-white sticky top-0 z-10">
        <div>
            ...
            <p class="text-lg font-bold text-gray-900">
                <?php echo e($payment->reference_no ?? '#' . str_pad($payment->id, 6, '0', STR_PAD_LEFT)); ?>
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
            <div class="flex flex-col items-center text-center">
                ...
                <h3 class="mt-3 text-xl font-bold text-gray-900 leading-tight">
                    <?php echo e($payment->resident?->first_name ?? 'Unknown'); ?> <?php echo e($payment->resident?->last_name ?? 'Resident'); ?>

                </h3>
                <p class="text-sm text-gray-500 mt-1">
                    Block <?php echo e($payment->resident?->block ?? '-'); ?> • Lot <?php echo e($payment->resident?->lot ?? '-'); ?>

                </p>
                
                <div class="flex gap-3 mt-4">
                    <?php if($payment->resident): ?>
                    <a href="<?php echo e(route('admin.residents.show', $payment->resident->id)); ?>"
...
<div class="bg-gray-50 rounded-2xl p-3 border border-gray-200 text-center">
                    ...
                    <p class="text-lg font-bold text-green-700 font-mono">₱<?php echo e(number_format($payment->amount, 2)); ?></p>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="text-sm font-bold text-gray-900"><?php echo e(\Carbon\Carbon::parse($payment->date_paid)->format('M d, Y')); ?></p>
                    </div>
                     <div>
                        <p class="text-xs font-semibold text-gray-500 uppercase mb-1">Method</p>
                        <p class="text-sm font-bold text-gray-900 capitalize"><?php echo e($payment->payment_method); ?></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <?php if($payment->proof): ?>
                <div class="rounded-xl overflow-hidden border border-gray-200 group relative shadow-sm">
                    <img src="<?php echo e(Storage::disk('public')->url($payment->proof)); ?>" 
                         class="w-full h-auto object-cover">

                    <a href="<?php echo e(Storage::disk('public')->url($payment->proof)); ?>" target="_blank"
    ...
    <div class="p-6 border-t border-gray-100 bg-white z-10 flex items-center gap-3">
        <form action="<?php echo e(route('admin.payments.reject', $payment->id)); ?>" method="POST" class="flex-1">
            <?php echo csrf_field(); ?>
        ...
        <form action="<?php echo e(route('admin.payments.approve', $payment->id)); ?>" method="POST" class="flex-1">
            <?php echo csrf_field(); ?>
...
<?php /**PATH C:\Users\Sheehan\subdivision-dues-system-final\resources\views\admin\payments\partials\drawer.blade.php ENDPATH**/ ?>