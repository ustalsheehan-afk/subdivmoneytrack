<?php $__env->startSection('title', 'Activity Logs & Monitoring'); ?>
<?php $__env->startSection('page-title', 'Activity Logs & Monitoring'); ?>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="glass-card p-6 flex items-center gap-4">
            ...
            <div>
                ...
                <div class="text-3xl font-black text-gray-900"><?php echo e(number_format($todayCount)); ?></div>
                <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Last 24 hours</div>
            </div>
        </div>
        <div class="glass-card p-6 flex items-center gap-4 <?php echo e($failedLogins > 0 ? 'border border-red-100' : ''); ?>">
            <div class="w-12 h-12 rounded-2xl <?php echo e($failedLogins > 0 ? 'bg-red-50 text-red-600 border border-red-100' : 'bg-gray-50 text-gray-400 border border-gray-100'); ?> flex items-center justify-center"><i class="bi bi-exclamation-octagon"></i></div>
            <div>
                <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Failed Login Attempts</div>
                <div class="text-3xl font-black <?php echo e($failedLogins > 0 ? 'text-red-600' : 'text-gray-900'); ?>"><?php echo e(number_format($failedLogins)); ?></div>
                <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Last 24 hours</div>
            </div>
        </div>
        <div class="glass-card p-6 flex items-center gap-4">
            ...
            <div>
                ...
                <div class="text-3xl font-black text-gray-900"><?php echo e(number_format($activeAdmins)); ?></div>
                <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Last 24 hours</div>
            </div>
        </div>
        <div class="glass-card p-6 flex items-center gap-4">
            ...
            <div>
                ...
                <div class="text-xl font-black text-gray-900"><?php echo e($topModule->module ?? 'N/A'); ?></div>
                <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Last 24 hours</div>
            </div>
        </div>
    </div>
    <div class="glass-card p-4">
        <div class="flex flex-col lg:flex-row lg:items-center gap-4">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="<?php echo e(route('admin.system.activity-logs.index')); ?>" class="px-4 py-2 bg-white border border-gray-200 rounded-xl text-[10px] font-black uppercase tracking-widest text-gray-600 hover:bg-gray-50">Clear Filters</a>
                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('export_logs')): ?>
                    <a href="<?php echo e(route('admin.system.activity-logs.export', request()->query())); ?>" class="px-4 py-2 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-700">Export Logs</a>
                <?php endif; ?>
            </div>
        </div>
    </div>
                ...
                <tbody class="divide-y divide-gray-50">
                    <?php $__empty_1 = true; $__currentLoopData = $logs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $log): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <?php
                            ...
                            $ip = $log->metadata['ip'] ?? '—';
                        ?>
                        <tr x-show="matchesSearch('<?php echo e(strtolower($userName.' '.$log->action.' '.$log->id)); ?>')">
                            <td class="px-6 py-4 text-gray-500"><?php echo e($log->created_at->format('M d, Y • h:i A')); ?></td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-gray-900 text-[#B6FF5C] flex items-center justify-center text-xs font-black"><?php echo e(strtoupper(substr($userName,0,1))); ?></div>
                                    <div class="leading-tight">
                                        <div class="text-sm font-black text-gray-900"><?php echo e(strtoupper($userName)); ?></div>
                                        <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest"><?php echo e($userRole); ?></div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border <?php echo e($conf['bg']); ?> <?php echo e($conf['text']); ?> <?php echo e($conf['border']); ?>"><i class="bi <?php echo e($conf['icon']); ?>"></i> <?php echo e(ucfirst($log->action)); ?></span>
                            </td>
                            <td class="px-6 py-4 font-bold text-gray-900"><?php echo e(ucfirst($log->module)); ?></td>
                            <td class="px-6 py-4 text-gray-600"><?php echo e($log->description); ?></td>
                            <td class="px-6 py-4 text-gray-500"><?php echo e($ip); ?></td>
                            <td class="px-6 py-4 text-right">
                                <button @click="openDetails(<?php echo e(json_encode(['id'=>$log->id,'user'=>$userName,'role'=>$userRole,'action'=>$log->action,'module'=>$log->module,'description'=>$log->description,'ip'=>$ip,'time'=>$log->created_at->format('M d, Y • h:i A'),'metadata'=>$log->metadata])); ?>)" class="px-4 py-2 bg-gray-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-black">View Details</button>
                            </td>
                        </tr>
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\Sheehan\subdivision-dues-system-final\resources\views\admin\system\activity-logs.blade.php ENDPATH**/ ?>