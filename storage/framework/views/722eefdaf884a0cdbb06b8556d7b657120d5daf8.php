<?php $__env->startSection('title', 'Invitation Management'); ?>
<?php $__env->startSection('page-title', 'Manage Invitations'); ?>

<?php $__env->startSection('content'); ?>
<div class="space-y-8 animate-fade-in pb-20" x-data="{ 
    selectedId: <?php echo e($invitations->first()->id ?? 'null'); ?>,
    loading: false,
    invitation: null,
    search: '',
    previewTab: 'email',
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
        <div class="flex items-center gap-3">
            ...
            <p class="text-sm font-medium text-amber-800"><?php echo e(session('warning')); ?></p>
        </div>
    </div>
    <?php endif; ?>
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-sm font-medium text-amber-800">Warning: <?php echo e($stats['expiring_soon']); ?> invitation<?php echo e($stats['expiring_soon'] > 1 ? 's' : ''); ?> will expire within 24 hours.</p>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="glass-card p-6 relative overflow-hidden group hover:scale-[1.02] transition-all duration-300">
            ...
            <div class="absolute left-0 top-0 bottom-0 w-1 bg-<?php echo e($card['color'] === 'emerald' ? 'emerald-500' : ($card['color'] === 'amber' ? 'amber-500' : ($card['color'] === 'blue' ? 'blue-500' : 'red-500'))); ?>"></div>
            
            <div class="flex items-center gap-5 relative z-10">
                <div class="w-14 h-14 bg-<?php echo e($card['color']); ?>-50 rounded-2xl flex items-center justify-center text-<?php echo e($card['color']); ?>-600 shadow-sm border border-<?php echo e($card['color']); ?>-100/50 group-hover:bg-<?php echo e($card['color'] === 'emerald' ? 'emerald-600' : ($card['color'] === 'amber' ? 'amber-500' : ($card['color'] === 'blue' ? 'blue-500' : 'red-500'))); ?> group-hover:text-white transition-all duration-500">
                    <i class="bi <?php echo e($card['icon']); ?> text-2xl"></i>
                </div>
                <div>
                    <p class="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1"><?php echo e($card['label']); ?></p>
                    <span class="text-3xl font-black text-gray-900 tracking-tight"><?php echo e(number_format($card['value'])); ?></span>
                </div>
            </div>
        </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr class="group cursor-pointer transition-all duration-300 relative"
                                :class="selectedId === <?php echo e($invite->id); ?> ? 'bg-emerald-50/30' : 'hover:bg-gray-50/80'"
                                @click="selectInvitation(<?php echo e($invite->id); ?>)"
                                x-show="'<?php echo e(strtolower($invite->first_name . ' ' . $invite->last_name . ' ' . $invite->email . ' ' . $invite->status)); ?>'.includes(search.toLowerCase())">
                                
                                
                                <div x-show="selectedId === <?php echo e($invite->id); ?>" 
                                ...
                                <td class="px-8 py-5">
                                    <div class="flex items-center gap-4">
                                        <div class="w-12 h-12 rounded-2xl bg-gray-900 flex items-center justify-center text-emerald-400 font-black text-sm shadow-lg shadow-gray-900/10 group-hover:scale-110 transition-transform duration-300">
                                            <?php echo e(strtoupper(substr($invite->first_name, 0, 1) . substr($invite->last_name, 0, 1))); ?>

                                        </div>
                                        <div>
                                            <p class="text-sm font-extrabold text-gray-900 leading-none mb-1 group-hover:text-emerald-900 transition-colors"><?php echo e($invite->first_name); ?> <?php echo e($invite->last_name); ?></p>
                                            <p class="text-[11px] font-bold text-gray-400 uppercase tracking-wider"><?php echo e($invite->email); ?></p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-5">
                                    ...
                                    ?>
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border <?php echo e($conf['bg']); ?> <?php echo e($conf['text']); ?> <?php echo e($conf['border']); ?>">
                                        <?php echo e($status); ?>
                                ...
                                <td class="px-6 py-5">
                                    <div class="flex items-center gap-3">
                                        <i class="bi bi-envelope-fill text-sm <?php echo e($invite->email_status === 'sent' ? 'text-blue-500' : 'text-gray-200'); ?>" title="Email"></i>
                                        <i class="bi bi-chat-left-dots-fill text-sm <?php echo e($invite->sms_status === 'sent' ? 'text-emerald-500' : 'text-gray-200'); ?>" title="SMS"></i>
                                    </div>
                                </td>
                                <td class="px-6 py-5">
                                    <div class="flex flex-col">
                                        <span class="text-[11px] font-black text-gray-900 uppercase tracking-tighter">
                                            <?php echo e($invite->expires_at->format('M d, Y')); ?>

                                        </span>
                                        <?php if($status === 'pending'): ?>
                                            <span class="text-[9px] font-bold <?php echo e($isExpired ? 'text-red-400' : 'text-amber-500'); ?> uppercase tracking-widest mt-0.5">
                                                <?php echo e($isExpired ? 'Expired' : $invite->expires_at->diffForHumans()); ?>
...
<div id="invite-modal" 
    ...
    <div x-show="show"
        ...
        <form id="create-invite-form" action="<?php echo e(route('admin.invitations.store')); ?>" method="POST" class="p-10 space-y-8">
            <?php echo csrf_field(); ?>
...
//     try {
//         const response = await fetch("<?php echo e(route('admin.invitations.store')); ?>", {
//             method: 'POST',
//             headers: {
//                 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
//                 'Accept': 'application/json'
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\Sheehan\subdivision-dues-system-final\resources\views/admin/invitations/index.blade.php ENDPATH**/ ?>