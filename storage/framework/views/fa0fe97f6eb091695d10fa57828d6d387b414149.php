<?php $__env->startSection('title', 'Create Billing Statement'); ?>
...
<div class="space-y-8 animate-fade-in pb-20">
    ...
    <form id="billing-form" action="<?php echo e(route('admin.dues.store')); ?>" method="POST" x-data="billingForm()">
        <?php echo csrf_field(); ?>
                        ...
                        <div class="flex flex-col border border-gray-100 rounded-3xl overflow-hidden bg-gray-50/30">
                            ...
                            <div class="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar" id="available-list">
                                ...
                                <div class="resident-item p-4 rounded-2xl bg-white border border-gray-50 flex items-center justify-between group hover:border-emerald-500/30 hover:bg-emerald-50/30 hover:shadow-sm transition-all duration-300" 
                                    data-id="<?php echo e($resident->id); ?>" data-name="<?php echo e(strtolower($resident->first_name . ' ' . $resident->last_name)); ?>" data-unit="<?php echo e(strtolower('B'.$resident->block.'/L'.$resident->lot)); ?>">
                                    <div>
                                        <div class="text-sm font-bold text-gray-900 group-hover:text-emerald-700 transition-colors"><?php echo e($resident->first_name); ?> <?php echo e($resident->last_name); ?></div>
                                        <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-0.5">B<?php echo e($resident->block); ?> / L<?php echo e($resident->lot); ?></div>
                                    </div>
                                    <button type="button" onclick="addResident(<?php echo e($resident->id); ?>)" class="w-9 h-9 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center opacity-0 group-hover:opacity-100 hover:bg-emerald-600 hover:text-white transition-all shadow-sm">
...
<script>
    ...
    const allResidents = <?php echo json_encode($residents, 15, 512) ?>;
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\Sheehan\subdivision-dues-system-final\resources\views\admin\dues\create.blade.php ENDPATH**/ ?>