<?php $__env->startSection('title', 'Edit Amenity'); ?>
<?php $__env->startSection('page-title', 'Edit Amenity'); ?>
...
<div class="space-y-8 animate-fade-in pb-20">
            ...
            <div class="flex items-center gap-6">
                ...
                <div>
                    ...
                    <p class="mt-2 text-gray-600 text-lg max-w-xl">
                        Update details for <span class="font-bold text-gray-900"><?php echo e($amenity->name); ?></span>.
                    </p>
                </div>
            </div>
    ...
    <form id="amenity-form" action="<?php echo e(route('admin.amenities.update', $amenity)); ?>" method="POST" enctype="multipart/form-data" 
          x-data="{ 
              equipmentList: <?php echo e(json_encode($amenity->equipment ?? [])); ?>,
              addEquipment() { this.equipmentList.push({ name: '', price: '' }); },
              removeEquipment(index) { this.equipmentList.splice(index, 1); },
              imagePreview: <?php echo e(json_encode($amenity->image ? Storage::url($amenity->image) : null)); ?>,
              fileChosen(event) {
                ...
                <section class="glass-card p-8 space-y-8 relative overflow-hidden group">
                    ...
                    <div class="grid grid-cols-1 gap-8">
                        <div class="space-y-3">
                            ...
                            <input type="text" name="name" value="<?php echo e(old('name', $amenity->name)); ?>" 
                        ...
                        <div class="space-y-3">
                            ...
                            <textarea name="description" rows="4" 
                                ...
                                placeholder="Provide a brief description of the amenity and its features..."><?php echo e(old('description', $amenity->description)); ?></textarea>
                        </div>
                    </div>
                </section>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <?php $selectedDays = old('days_available', $amenity->days_available ?? []); ?>
                                <?php $__currentLoopData = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $day): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <label class="cursor-pointer group">
                                        <input type="checkbox" name="days_available[]" value="<?php echo e($day); ?>" class="peer sr-only"
                                            <?php echo e(in_array($day, $selectedDays) ? 'checked' : ''); ?>>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-3 gap-2">
                                <?php $selectedSlots = old('time_slots', $amenity->time_slots ?? []); ?>
                                <?php $__currentLoopData = ['Morning', 'Afternoon', 'Evening']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $slot): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <label class="cursor-pointer group">
                                        <input type="checkbox" name="time_slots[]" value="<?php echo e($slot); ?>" class="peer sr-only"
                                               <?php echo e(in_array($slot, $selectedSlots) ? 'checked' : ''); ?>>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t border-gray-50">
                        <div class="space-y-3">
                            ...
                            <div class="relative group/input">
                                ...
                                <input type="number" name="max_capacity" value="<?php echo e(old('max_capacity', $amenity->max_capacity)); ?>" min="1" required
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="relative group/input">
                                ...
                                <input type="number" name="buffer_minutes" value="<?php echo e(old('buffer_minutes', $amenity->buffer_minutes)); ?>" min="0" step="5"
                                    class="w-full pl-12 pr-6 py-4 rounded-2xl border border-gray-200 bg-gray-50/50 text-sm font-black focus:bg-white focus:border-emerald-500 transition-all outline-none tabular-nums">
                    ...
                    <div class="relative z-10 space-y-6">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="relative group/price">
                                ...
                                <input type="number" name="price" value="<?php echo e(old('price', $amenity->price)); ?>" step="0.01" min="0" required
                ...
                <div class="glass-card p-8 space-y-8">
                    <div class="space-y-4">
                        ...
                        <div class="relative group/select">
                            <select name="status" class="w-full px-5 py-4 rounded-2xl border border-gray-200 bg-gray-50/50 text-[10px] font-black uppercase tracking-widest text-gray-700 appearance-none focus:bg-white focus:border-emerald-500 transition-all outline-none cursor-pointer">
                                <option value="active" <?php echo e(in_array(old('status', $amenity->status), ['active', '1', 1, true]) ? 'selected' : ''); ?>>Active (Public)</option>
                                <option value="maintenance" <?php echo e(old('status', $amenity->status) === 'maintenance' ? 'selected' : ''); ?>>Maintenance</option>
                                <option value="inactive" <?php echo e(in_array(old('status', $amenity->status), ['inactive', '0', 0, false]) ? 'selected' : ''); ?>>Inactive (Hidden)</option>
                            </select>
                    ...
                    <label class="flex items-center gap-4 cursor-pointer group p-5 bg-emerald-50/30 rounded-2xl border border-emerald-100 hover:bg-emerald-50 transition-all shadow-sm">
                        <div class="relative flex items-center">
                            ...
                            <input type="checkbox" name="highlight" value="1" class="peer h-6 w-6 rounded-lg border-emerald-300 text-emerald-600 focus:ring-emerald-500/20 transition-all" <?php echo e(old('highlight', $amenity->highlight) ? 'checked' : ''); ?>>
                        </div>
                        <div>
                    ...
                    <div class="p-5 bg-gray-50/50 rounded-2xl border border-gray-100 flex items-center gap-4 group/pdf">
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="text-[10px] font-black text-gray-900 uppercase tracking-widest truncate" id="pdf-filename">
                                <?php echo e($amenity->pdf_rules ? basename($amenity->pdf_rules) : 'No file selected'); ?>

                            </p>
                            <p class="text-[9px] font-bold text-gray-400 uppercase tracking-tighter mt-0.5">Guidelines & Policies</p>
                        </div>
                        <?php if($amenity->pdf_rules): ?>
                            <a href="<?php echo e(Storage::url($amenity->pdf_rules)); ?>" target="_blank" class="text-[10px] font-black text-emerald-600 uppercase tracking-widest hover:text-emerald-700 transition-colors">View</a>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\Sheehan\subdivision-dues-system-final\resources\views\admin\amenities\edit.blade.php ENDPATH**/ ?>