...
<div class="space-y-8 animate-fade-in">
    ...
    <div class="glass-card p-8 relative overflow-hidden group">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
            ...
            <div class="flex items-center gap-3">
                <form action="{{ route('admin.messages.notifications.markAllRead') }}" method="POST">
                    @csrf
    ...
    <div class="glass-card p-4 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div class="flex flex-wrap items-center gap-4 flex-1">
            <form action="{{ route('admin.messages.notifications.index') }}" method="GET" class="flex flex-wrap items-center gap-4 w-full">
                {{-- Category Filter --}}
                ...
                @if(request()->anyFilled(['type', 'status']))
                    <a href="{{ route('admin.messages.notifications.index') }}" class="h-11 w-11 flex items-center justify-center rounded-xl border border-red-100 text-red-500 hover:bg-red-50 transition-all" title="Clear Filters">
                ...
                <div class="p-8 flex items-center justify-between hover:bg-emerald-50/20 transition-all group relative border-l-4 {{ !$notification->is_read ? 'bg-emerald-50/10 border-emerald-500' : 'border-transparent' }}">
                    ...
                    <div class="flex items-center gap-3">
                        @if($notification->link)
                            <form action="{{ route('admin.messages.notifications.read', $notification->id) }}" method="POST">
                                @csrf
