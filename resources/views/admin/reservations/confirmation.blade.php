...
@section('title', 'Reservation Created')
@section('page-title', 'Reservation Created')
...
<div class="max-w-4xl mx-auto p-4 md:p-8">
    <div class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden text-center p-8 md:p-12">
        ...
        <p class="text-lg text-gray-600 mb-8 max-w-2xl mx-auto">
            ...
            <span class="font-black text-gray-900">{{ $reservation->customer_name }}</span>.
        </p>
        ...
        <div class="bg-gray-50 rounded-3xl p-6 md:p-8 text-left mb-8 border border-gray-200">
            <div class="flex flex-col md:flex-row md:items-start justify-between gap-6 border-b border-gray-200 pb-5 mb-6">
                <div>
                    ...
                    <p class="font-black text-2xl text-gray-900 font-mono tracking-wider">{{ $reservation->reference_code }}</p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="font-black text-lg text-gray-900">{{ $reservation->amenity->name }}</p>
                </div>
                <div>
                    <p class="text-gray-500 text-sm mb-1">Customer Type</p>
                    <p class="font-black text-lg text-gray-900">{{ $reservation->customer_type_label }}</p>
                </div>
                <div>
                    <p class="text-gray-500 text-sm mb-1">Customer</p>
                    <p class="font-black text-gray-900">{{ $reservation->customer_name }}</p>
                    <p class="text-sm text-gray-500 mt-1">{{ $reservation->customer_contact }}</p>
                ...
                <div>
                    ...
                    <p class="font-black text-gray-900">{{ \Carbon\Carbon::parse($reservation->date)->format('F j, Y') }}</p>
                    <p class="text-sm text-emerald-700 font-bold mt-1">{{ $reservation->time_slot }}</p>
                </div>
                <div>
                    <p class="text-gray-500 text-sm mb-1">Guests</p>
                    <p class="font-black text-gray-900">{{ $reservation->guest_count }} People</p>
                </div>
                <div>
                    <p class="text-gray-500 text-sm mb-1">Total Amount</p>
                    <p class="font-black text-2xl text-emerald-700">₱{{ number_format($reservation->total_price, 2) }}</p>
                </div>
            </div>

            <div class="mt-6 p-4 rounded-xl {{ $reservation->payment_status === 'paid' ? 'bg-emerald-100 border border-emerald-200' : 'bg-amber-50 border border-amber-200' }}">
        ...
        <div class="flex flex-col md:flex-row gap-4 justify-center">
            <a href="{{ route('admin.amenity-reservations.index', ['date' => $reservation->date->format('Y-m-d'), 'active_id' => $reservation->id]) }}" class="px-8 py-4 bg-[#081412] text-[#B6FF5C] font-black text-lg rounded-xl transition-all flex items-center justify-center gap-2 border border-[#B6FF5C]/20">
                <i class="bi bi-eye"></i> View Reservation
            </a>
            <a href="{{ route('admin.amenity-reservations.create') }}" class="px-8 py-4 bg-white text-gray-700 font-black text-lg rounded-xl border-2 border-gray-200 hover:bg-gray-50 transition-all flex items-center justify-center gap-2">
