...
<div class="max-w-5xl mx-auto space-y-6" x-data="adminReservationWizard()">
    ...
    <a href="{{ route('admin.amenity-reservations.index') }}" class="inline-flex items-center gap-3 text-[11px] font-black uppercase tracking-widest text-gray-400 hover:text-emerald-600 transition-all">
    ...
    <div class="bg-white rounded-[32px] shadow-xl border border-gray-100 overflow-hidden">
        <form method="POST" action="{{ route('admin.amenity-reservations.store') }}" class="p-8 space-y-8">
            @csrf
                    ...
                    <div class="xl:col-span-2 space-y-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <input type="date" name="date" x-model="date" @change="checkAvailability()" min="{{ now()->format('Y-m-d') }}" class="w-full p-5 rounded-3xl border-2 border-gray-100 bg-gray-50 font-black focus:border-emerald-500 focus:ring-0">
                            </div>
                            <div>
...
function adminReservationWizard() {
    return {
        ...
        async checkAvailability() {
            ...
            try {
                const response = await fetch(`{{ url('admin/amenity-reservations') }}/${this.selectedAmenityId}/unavailable-slots?date=${this.date}`);
