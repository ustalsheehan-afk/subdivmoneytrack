...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.dues.create') }}" class="px-5 py-2.5 bg-blue-600 text-white text-sm font-bold rounded-xl hover:bg-blue-700 transition-all shadow-md shadow-blue-100 flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow group">
            ...
            <div class="text-2xl font-black text-gray-900 mb-1">{{ $totalActiveDues }}</div>
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow group">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="px-2.5 py-1 rounded-lg bg-blue-50 text-blue-700 text-[10px] font-bold uppercase tracking-wider border border-blue-100">
                    {{ number_format(($totalCollected / max(1, $totalExpected)) * 100, 1) }}%
                </span>
            </div>
            <div class="text-2xl font-black text-gray-900 mb-1">₱{{ number_format($totalCollected, 2) }}</div>
            <div class="flex items-center gap-1.5 text-gray-400 text-xs font-medium">
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow group text-orange-600">
            ...
            <div class="text-2xl font-black text-gray-900 mb-1">₱{{ number_format($pendingCollection, 2) }}</div>
            <div class="flex items-center gap-1.5 text-gray-400 text-xs font-medium">
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow group">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="px-2.5 py-1 rounded-lg {{ $growth >= 0 ? 'bg-green-50 text-green-700 border-green-100' : 'bg-red-50 text-red-700 border-red-100' }} text-[10px] font-bold uppercase tracking-wider border">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-50 flex items-center justify-between">
            ...
            <a href="{{ route('admin.dues.index') }}" class="text-sm font-bold text-blue-600 hover:text-blue-700 flex items-center gap-1 transition-all">
                <span>View All</span>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="group hover:bg-blue-50/30 transition-all duration-200">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <div class="text-sm font-bold text-gray-900">{{ $batch->title }}</div>
                                    <div class="text-[11px] text-gray-400 font-bold uppercase tracking-wider">{{ str_replace('_', ' ', $batch->type) }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <div class="text-sm font-medium text-gray-700">{{ $batch->billing_period_start ? $batch->billing_period_start->format('M Y') : 'N/A' }}</div>
                            <div class="text-[10px] text-gray-400 font-bold uppercase">Due: {{ $batch->due_date ? $batch->due_date->format('M d, Y') : 'N/A' }}</div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="w-48">
                                <div class="flex items-center justify-between mb-1.5">
                                    <span class="text-[11px] font-bold text-gray-500">{{ number_format($batch->progress, 0) }}%</span>
                                    <span class="text-[10px] text-gray-400">₱{{ number_format($batch->collected_amount, 0) }} / ₱{{ number_format($batch->total_expected, 0) }}</span>
                                </div>
                                <div class="w-full bg-gray-100 rounded-full h-1.5 overflow-hidden">
                                    <div class="bg-blue-500 h-full rounded-full transition-all duration-1000" style="width: {{ $batch->progress }}%"></div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <a href="{{ route('admin.dues.show', $batch) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 text-xs font-bold rounded-xl hover:bg-blue-600 hover:text-white transition-all shadow-sm">
                                <i class="bi bi-eye"></i>
                                <span>Details</span>
                            </a>
                        </td>
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-20 text-center">
                            ...
                            <div class="mt-4">
                                <a href="{{ route('admin.dues.create') }}" class="px-5 py-2.5 bg-blue-600 text-white text-xs font-bold rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-100 transition-all">
