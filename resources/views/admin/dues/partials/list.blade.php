{{-- Dues List Partial --}}
@if(count($groupedDues) > 0)
    @foreach($groupedDues as $monthLabel => $batchList)
        <div x-data="{ expanded: true }" class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6">
            ...
            <button @click="expanded = !expanded" class="w-full flex items-center justify-between px-6 py-4 bg-gray-50/50 hover:bg-gray-50 transition border-b border-gray-100">
                <div class="flex items-center gap-4">
                    ...
                    <span class="px-2.5 py-0.5 rounded-full bg-gray-200 text-gray-600 text-xs font-medium">{{ count($batchList) }} Dues</span>
                    
                    @if($monthLabel === now()->format('F Y'))
                                ...
                                <tr class="hover:bg-gray-50/50 transition group">
                                    <td class="px-6 py-4">
                                        <div class="flex flex-col">
                                            <span class="font-bold text-gray-900">{{ $due->title }}</span>
                                            <span class="text-xs text-gray-500 mt-0.5">{{ $due->description }}</span>
                                            <span class="inline-block mt-2 px-2 py-0.5 rounded text-[10px] font-bold uppercase w-fit border {{ $colorClass }}">
                                                {{ str_replace('_', ' ', $due->type) }}
                                            </span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-gray-600">
                                        @if($due->billing_period_start)
                                            {{ \Carbon\Carbon::parse($due->billing_period_start)->format('M d') }} - 
                                            {{ \Carbon\Carbon::parse($due->billing_period_end)->format('M d') }}
                                        @else
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex flex-col items-center">
                                            <span class="font-bold text-gray-800">{{ \Carbon\Carbon::parse($due->due_date)->format('d') }}</span>
                                            <span class="text-xs text-gray-500 uppercase">{{ \Carbon\Carbon::parse($due->due_date)->format('M') }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="w-full max-w-[140px]">
                                            <div class="flex justify-between text-xs mb-1">
                                                <span class="font-semibold text-gray-700">{{ $due->paid_residents }}/{{ $due->total_residents }} Paid</span>
                                                <span class="font-bold {{ $progress >= 100 ? 'text-emerald-600' : 'text-blue-600' }}">{{ round($progress) }}%</span>
                                            </div>
                                            <div class="w-full bg-gray-100 rounded-full h-2">
                                                <div class="h-2 rounded-full transition-all duration-500 {{ $progress >= 100 ? 'bg-emerald-500' : 'bg-blue-500' }}" style="width: {{ $progress }}%"></div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            @if(!$due->archived_at)
                                                <form action="{{ route('admin.dues.archive', $due->batch_id) }}" method="POST" onsubmit="return confirm('Archive this batch?');">
                                                    @csrf
                                                ...
                                                <form action="{{ route('admin.dues.destroy', $due->batch_id) }}" method="POST" onsubmit="return confirm('Delete this batch? This will remove dues for ALL residents.');">
                                                    @csrf
                                                    @method('DELETE')
