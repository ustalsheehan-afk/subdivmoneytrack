...
document.addEventListener('DOMContentLoaded', function () {
    ...
    /* ===============================
       GLOBAL STATE
    =============================== */
    let currentPage = {{ $residents->currentPage() }};
    let hasMorePages = {{ $residents->hasMorePages() ? 'true' : 'false' }};
    let isLoading = false;
    let currentBlock = "{{ request('block') }}";
    let currentView = '{{ request("view", "list") }}';
    let isSelectionMode = false;
    let searchTimeout;
    let activeResidentId = "{{ request('active_id') }}";
    ...
    window.applyAllFilters = function () {
        ...
        window.location.href = `{{ route('admin.residents.index') }}?${params.toString()}`;
    }
