...
<tr onclick="loadRequestDetails({{ json_encode($requestData) }})" 
    ...
    <td class="p-6 align-middle text-left">
        <div class="flex items-center gap-4">
            ...
            <div class="min-w-0">
                <p class="text-sm font-black text-gray-900 group-hover:text-brand-accent transition truncate">{{ $req->resident?->full_name ?? 'Unknown Resident' }}</p>
                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-tight">Blk {{ $req->resident?->block ?? '-' }} • Lot {{ $req->resident?->lot ?? '-' }}</p>
            </div>
        </div>
    </td>
    ...
    <td class="p-6 text-center align-middle">
        <span class="text-[10px] font-black text-gray-500 uppercase tracking-widest bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">{{ $req->type }}</span>
    </td>

    {{-- Date --}}
    <td class="p-6 text-center align-middle">
        <div class="flex flex-col items-center">
            <span class="text-sm font-black text-gray-900 tracking-tight">{{ $req->created_at->format('M d, Y') }}</span>
            <span class="text-[10px] font-bold text-gray-400 uppercase mt-0.5">{{ $req->created_at->diffForHumans() }}</span>
        </div>
    </td>

    {{-- Priority --}}
    <td class="p-6 text-center align-middle">
        <span class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-[10px] font-black border uppercase tracking-widest {{ $priorityClass }}">
    ...
    {{-- Status --}}
    <td class="p-6 text-center align-middle">
        <span class="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-[10px] font-black border uppercase tracking-widest {{ $statusStyles['bg'] }} {{ $statusStyles['text'] }} {{ $statusStyles['border'] }}">
