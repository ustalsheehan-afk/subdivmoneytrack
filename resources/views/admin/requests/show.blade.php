...
Initial request submitted by {{ $request->resident->first_name }}
</p>
...
<div class="absolute -left-[26px] top-1 w-4 h-4 rounded-full {{ $isInProgress ? 'bg-amber-500' : 'bg-gray-300' }} border-4 border-white shadow"></div>
...
{{ $progressDate->format('F d, Y • g:i A') }}
</p>
@endif
...
<div class="absolute -left-[26px] top-1 w-4 h-4 rounded-full {{ $isCompleted ? 'bg-emerald-500' : 'bg-gray-300' }} border-4 border-white shadow"></div>
...
{{ $completedDate->format('F d, Y • g:i A') }}
</p>
@endif
...
<img
src="{{ $request->resident->photo ? asset('storage/'.$request->resident->photo) : asset('CDlogo.jpg') }}"
...
Block {{ $request->resident->block }} • Lot {{ $request->resident->lot }}
</p>
</div>
