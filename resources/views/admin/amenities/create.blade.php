...
@section('page-title', 'Add New Amenity')
...
<div class="space-y-8 animate-fade-in pb-20">
    ...
    <form id="amenity-form" action="{{ route('admin.amenities.store') }}" method="POST" enctype="multipart/form-data" 
          x-data="{ 
              equipmentList: [],
                    ...
                    <div class="grid grid-cols-1 gap-8">
                        ...
                        <div class="space-y-3">
                            ...
                            <textarea name="description" rows="4" 
                                ...
                                placeholder="Provide a brief description of the amenity and its features...">{{ old('description') }}</textarea>
                        </div>
                    </div>
                </section>
                                ...
                                @foreach(['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'] as $day)
                                    <label class="cursor-pointer group">
                                        <input type="checkbox" name="days_available[]" value="{{ $day }}" class="peer sr-only"
                                            {{ in_array($day, old('days_available', ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'])) ? 'checked' : '' }}>
                                ...
                                @foreach(['Morning', 'Afternoon', 'Evening'] as $slot)
                                    <label class="cursor-pointer group">
                                        <input type="checkbox" name="time_slots[]" value="{{ $slot }}" class="peer sr-only"
                                               {{ in_array($slot, old('time_slots', ['Morning', 'Afternoon', 'Evening'])) ? 'checked' : '' }}>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t border-gray-50">
                        <div class="space-y-3">
                            ...
                            <div class="relative group/input">
                                ...
                                <input type="number" name="max_capacity" value="{{ old('max_capacity', 1) }}" min="1" required
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="relative group/input">
                                ...
                                <input type="number" name="buffer_minutes" value="{{ old('buffer_minutes', 30) }}" min="0" step="5"
                                    class="w-full pl-12 pr-6 py-4 rounded-2xl border border-gray-200 bg-gray-50/50 text-sm font-black focus:bg-white focus:border-emerald-500 transition-all outline-none tabular-nums">
                ...
                <div class="glass-card p-8 space-y-8">
                    ...
                    <label class="flex items-center gap-4 cursor-pointer group p-5 bg-emerald-50/30 rounded-2xl border border-emerald-100 hover:bg-emerald-50 transition-all shadow-sm">
                        <div class="relative flex items-center">
                            ...
                            <input type="checkbox" name="highlight" value="1" class="peer h-6 w-6 rounded-lg border-emerald-300 text-emerald-600 focus:ring-emerald-500/20 transition-all" {{ old('highlight') == '1' ? 'checked' : '' }}>
                        </div>
                        <div>
