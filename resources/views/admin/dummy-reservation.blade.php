...
@section('title', 'Reservation Layout Preview')
...
<div class="p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 h-[calc(100vh-12rem)]">
        @foreach($amenities as $amenity)
            ...
            <!-- Amenity Column -->
            <div class="flex flex-col bg-white rounded-xl shadow-sm border {{ $colBorder }} overflow-hidden h-full">
                <!-- Column Header -->
                <div class="p-4 {{ $headerBg }} border-b {{ $colBorder }} flex justify-between items-center">
                    <h2 class="font-bold {{ $headerText }} text-lg flex items-center gap-2">
                        <span class="w-3 h-3 rounded-full bg-current"></span>
                        {{ $amenity['name'] }}
                    </h2>
                    <span class="text-xs font-semibold {{ $headerText }} bg-white bg-opacity-50 px-2 py-1 rounded-full">
                        {{ count($amenity['reservations']) }} Reservations
                    </span>
                </div>

                <!-- Reservations List (Vertical Stack) -->
                <div class="flex-1 overflow-y-auto p-4 {{ $colBg }} space-y-3">
                    @forelse($amenity['reservations'] as $reservation)
                        @php
                        ...
                        <!-- Reservation Card -->
                        <div class="group relative p-3 rounded-lg border {{ $statusClasses }} transition-all duration-200 shadow-sm hover:shadow-md cursor-pointer">
                            ...
                            <div class="absolute left-1/2 -translate-x-1/2 -top-10 hidden group-hover:block z-50 whitespace-nowrap">
                                <div class="bg-gray-800 text-white text-xs rounded py-1 px-2 shadow-lg">
                                    {{ $reservation['resident_name'] }} | {{ $reservation['time_slot'] }} | {{ ucfirst($status) }}
                                    <div class="absolute -bottom-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-gray-800 rotate-45"></div>
                                </div>
                            </div>

                            <div class="flex justify-between items-start mb-2">
                                <span class="font-semibold text-sm truncate pr-2">{{ $reservation['resident_name'] }}</span>
