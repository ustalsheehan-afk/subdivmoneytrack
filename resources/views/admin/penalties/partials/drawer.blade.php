<div class="h-full flex flex-col bg-white">
    ...
    <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-white sticky top-0 z-10">
        <div>
            ...
            <p class="text-lg font-bold text-gray-900">
                #{{ str_pad($penalty->id, 5, '0', STR_PAD_LEFT) }}
            </p>
        </div>
    ...
    <div class="flex-1 overflow-y-auto p-6 space-y-6 bg-gray-50 custom-scrollbar">
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
            <div class="flex flex-col items-center text-center">
                <div class="relative p-1 rounded-full bg-gradient-to-tr from-[#800020]/30 to-transparent">
                    <img src="{{ $penalty->resident->photo && Storage::disk('public')->exists($penalty->resident->photo) ? Storage::disk('public')->url($penalty->resident->photo) : asset('CDlogo.jpg') }}"
                         class="w-24 h-24 rounded-full object-cover border-4 border-white shadow-md">
                </div>

                <h3 class="mt-3 text-xl font-bold text-gray-900">
                    {{ $penalty->resident->full_name ?? 'Unknown' }}
                </h3>
                <p class="text-sm text-gray-500">Block {{ $penalty->resident->block ?? '-' }} Lot {{ $penalty->resident->lot ?? '-' }}</p>
                
                <div class="flex gap-3 mt-4">
                    <a href="{{ route('admin.residents.show', $penalty->resident->id) }}"
        ...
        <div>
             <div class="flex items-center justify-between mb-2">
                ...
                @endphp
                <span class="px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wide {{ $colorClass }}">
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div class="p-4 rounded-xl bg-red-50 border border-red-100 text-center">
                    ...
                    <p class="text-lg font-bold text-red-700 font-mono">₱{{ number_format($penalty->amount, 2) }}</p>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm space-y-4">
                 <div>
                    ...
                    <p class="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap bg-gray-50 p-3 rounded-lg border border-gray-100">
                        {{ $penalty->reason ?? 'No description provided.' }}
                    </p>
                </div>
                <div class="pt-4 border-t border-gray-100 grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="text-sm font-bold text-gray-900">{{ $penalty->date_issued ? $penalty->date_issued->format('M d, Y') : '-' }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
                <div class="flex justify-between items-center mb-2">
                    ...
                    <a href="#" class="text-xs text-blue-600 hover:underline">ID: {{ $penalty->payment->id }}</a>
                </div>
                <p class="font-mono text-gray-900 font-bold text-lg">{{ $penalty->payment->or_number ?? 'No OR Number' }}</p>
                <p class="text-xs text-gray-500 mt-1">Paid on {{ $penalty->payment->date_paid ? $penalty->payment->date_paid->format('M d, Y') : '-' }}</p>
            </div>
        </div>
        @endif
    </div>
    ...
    <div class="p-6 border-t border-gray-100 bg-white sticky bottom-0 z-10 flex gap-3">
        <a href="{{ route('admin.penalties.edit', $penalty->id) }}" class="flex-1 bg-white border border-gray-300 text-gray-700 font-bold py-3 rounded-xl hover:bg-gray-50 transition shadow-sm text-center">
            Edit Penalty
        </a>
        <form action="{{ route('admin.penalties.destroy', $penalty->id) }}" method="POST" class="flex-1" onsubmit="return confirm('Are you sure you want to delete this penalty?');">
            @csrf
            @method('DELETE')
