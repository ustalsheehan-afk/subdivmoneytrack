...
<div class="max-w-3xl mx-auto px-4 py-8">
    <div class="mb-6">
        <a href="{{ route('resident.penalties.index') }}" class="text-sm font-bold text-slate-400 hover:text-blue-500 transition-colors flex items-center gap-2 uppercase tracking-widest">
    ...
    <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden animate-fade-in">
        ...
        <div class="p-8 border-b border-slate-50 flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-xl font-black text-slate-900 tracking-tight leading-tight">{{ $penalty->reason }}</h2>
                    <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-1">Issued on {{ $penalty->date_issued->format('M d, Y') }}</p>
                </div>
            </div>
            <div class="text-right">
                <span class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border
                    @if($penalty->status == 'paid') bg-emerald-50 text-emerald-600 border-emerald-100
                    @elseif($penalty->status == 'overdue') bg-red-50 text-red-600 border-red-100
                    @else bg-orange-50 text-orange-600 border-orange-100 @endif shadow-sm">
                    {{ strtoupper($penalty->status) }}
                </span>
            </div>
        </div>
        ...
        <div class="p-8 space-y-10">
                ...
                <div class="bg-slate-50/50 rounded-2xl border border-slate-100 overflow-hidden shadow-inner">
                    <div class="p-5 space-y-3">
                        <div class="flex justify-between items-center text-sm font-medium text-slate-600">
                            ...
                            <span class="font-bold text-slate-900">₱{{ number_format($penalty->amount, 2) }}</span>
                        </div>
                        ...
                        <div class="pt-3 border-t border-slate-200 flex justify-between items-center">
                            ...
                            <span class="text-xl font-black text-red-600">₱{{ number_format($penalty->amount, 2) }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="p-6 bg-red-50/30 border border-red-100/50 rounded-2xl space-y-4">
                    <ul class="space-y-3">
                        @if($penalty->type == 'late_payment')
                        <li class="flex gap-3 text-sm font-medium text-slate-600">
                            <i class="bi bi-dot text-red-500 text-xl leading-none"></i>
                            <span>A payment for <strong>'{{ $penalty->due->title ?? 'Association Fee' }}'</strong> was submitted after the due date.</span>
                        </li>
                        <li class="flex gap-3 text-sm font-medium text-slate-600">
                            <i class="bi bi-dot text-red-500 text-xl leading-none"></i>
                        ...
                        <li class="flex gap-3 text-sm font-medium text-slate-600">
                            <i class="bi bi-dot text-red-500 text-xl leading-none"></i>
                            <span>Violation of subdivision rules and regulations regarding: <strong>{{ $penalty->reason }}</strong>.</span>
                        </li>
                        @endif
                    </ul>
                </div>
                ...
                <div class="p-6 bg-slate-50 rounded-2xl flex items-center justify-between border border-slate-100">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-black text-slate-900">{{ $penalty->due_date ? $penalty->due_date->format('F d, Y') : 'N/A' }}</p>
                        </div>
                    </div>
            ...
            {{-- 4. User Guidance & Action --}}
            <div class="space-y-4">
                <h3 class="text-[11px] font-black text-blue-500 uppercase tracking-widest px-1">What you can do</h3>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    @if($penalty->status != 'paid')
                    <a href="{{ route('resident.payments.pay', ['id' => $penalty->id, 'type' => 'penalty']) }}" class="flex flex-col p-5 bg-[#385780] hover:bg-[#2B3A4F] text-white rounded-2xl transition-all shadow-lg shadow-blue-900/10 group">
