...
@section('page-title', 'Payment: ' . ($type === 'penalty' ? $item->reason : $item->title))
...
<div class="max-w-4xl mx-auto p-6 lg:p-8">
    ...
    <a href="{{ route('resident.payments.index') }}" class="text-gray-500 hover:text-gray-700 font-medium text-sm flex items-center mb-6 transition-colors">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-gray-50 rounded-xl p-4 border border-gray-100 flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-[10px] font-bold text-blue-500 uppercase tracking-widest">{{ $type === 'penalty' ? 'Penalty Fee' : 'Auto-computed Balance' }}</p>
                    </div>
                    <p class="text-2xl font-bold text-gray-900">₱{{ number_format($type === 'penalty' ? $item->amount : $item->outstanding, 2) }}</p>
                </div>

                <form action="{{ route('resident.payments.process', ['id' => $item->id, 'type' => $type]) }}" method="POST" enctype="multipart/form-data" id="paymentForm">
                    @csrf
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" 
                               ...
                               class="w-full mt-1 px-4 py-2 border rounded-lg focus:ring focus:ring-blue-200 @error('reference_no') border-red-500 @enderror">
                        @error('reference_no')
