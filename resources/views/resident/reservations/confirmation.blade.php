...
@section('page-title', 'Reservation Confirmed')
...
<div class="max-w-3xl mx-auto p-4 md:p-8">
    ...
    <div class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden text-center p-8 md:p-12">
        ...
        <p class="text-lg text-gray-600 mb-8 max-w-lg mx-auto">
            Thank you, <span class="font-bold text-gray-900">{{ Auth::user()->name }}</span>. Your reservation request has been successfully submitted and is pending approval.
        </p>
        ...
        <div class="bg-gray-50 rounded-2xl p-6 md:p-8 text-left mb-8 border border-gray-200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="font-bold text-lg text-gray-900">{{ $reservation->amenity->name }}</p>
                </div>
                <div>
                    <p class="text-gray-500 text-sm mb-1">Reference No.</p>
                    <p class="font-bold text-lg text-gray-900 font-mono tracking-wider">#{{ str_pad($reservation->id, 6, '0', STR_PAD_LEFT) }}</p>
                </div>
                <div>
                    ...
                    <p class="font-bold text-gray-900">
                        ...
                        <br>
                        <span class="text-blue-600">{{ $reservation->time_slot }}</span>
                    </p>
                </div>
                <div>
                    <p class="text-gray-500 text-sm mb-1">Total Amount</p>
                    <p class="font-bold text-2xl text-blue-700">₱{{ number_format($reservation->total_price, 2) }}</p>
                </div>
            </div>

            <!-- Payment Status -->
            <div class="mt-6 p-4 rounded-xl {{ $reservation->payment_status === 'paid' ? 'bg-green-100 border border-green-200' : 'bg-yellow-50 border border-yellow-200' }}">
                <div class="flex items-start gap-3">
                    ...
                    @else
                        ...
                        <div>
                            <p class="font-bold text-yellow-800">Payment Pending ({{ ucfirst($reservation->payment_method) }})</p>
        ...
        <div class="flex flex-col md:flex-row gap-4 justify-center">
            <a href="{{ route('resident.my-reservations.index') }}" class="px-8 py-4 bg-blue-600 text-white font-bold text-lg rounded-xl hover:bg-blue-700 shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2">
                <i class="bi bi-calendar-check"></i> Go to My Reservations
            </a>
            <a href="{{ route('resident.dashboard') }}" class="px-8 py-4 bg-white text-gray-700 font-bold text-lg rounded-xl border-2 border-gray-200 hover:bg-gray-50 hover:border-gray-300 transition-all flex items-center justify-center gap-2">
