...
Modify your existing request details below.
</p>
</div>
...
<form action="{{ route('resident.requests.update', $requestItem->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
@csrf
@method('PATCH')
...
name="type"
id="type"
value="{{ old('type', $requestItem->type) }}"
required
...
class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-800 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-none">{{ old('description', $requestItem->description) }}</textarea>
...
<span id="photoLabel">{{ $requestItem->photo ? 'Change Reference Photo' : 'Upload Reference Photo' }}</span>
...
{{-- Image Preview --}}
<div id="imagePreviewContainer" class="{{ $requestItem->photo ? '' : 'hidden' }} pt-4 border-t border-gray-100 text-center">

<p class="text-xs text-gray-500 mb-3 font-medium">
{{ $requestItem->photo ? 'Current / New Photo Preview' : 'Photo Preview' }}
</p>

<div class="relative inline-block group">

<img id="imagePreview"
src="{{ $requestItem->photo ? asset('storage/' . $requestItem->photo) : '#' }}"
...
if (preview.src.endsWith('#')) {
    ...
    label.textContent = 'Change Reference Photo';
}
